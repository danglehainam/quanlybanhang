...
| `id` | INTEGER | PRIMARY KEY, AUTOINCREMENT | Khóa chính |
| `store_id` | INTEGER | NOT NULL, FOREIGN KEY (`stores.id`) | Thuộc cửa hàng nào |
| `created_by` | INTEGER | NOT NULL, FOREIGN KEY (`users.id`) | Ai tạo đơn hàng này |
| `customer_id` | INTEGER | NULLABLE, FOREIGN KEY (`customers.id`) | Khách hàng (null = khách lẻ) |
...
2. **Tổng Thu (Bán hàng)**: `SUM(final_amount)` từ bảng `orders` (nơi status = 1) theo ngày/tháng.
3. **Tổng Chi**: `SUM(amount)` từ bảng `transactions` (nơi type = 1) theo ngày/tháng.
4. **Khách hàng thân thiết**: `SELECT customer_id, COUNT(*) as order_count, SUM(final_amount) as total_spent FROM orders WHERE customer_id IS NOT NULL GROUP BY customer_id ORDER BY total_spent DESC`.
5. **Nhân viên bán chạy**: `SELECT created_by, COUNT(*) as order_count, SUM(final_amount) as total_revenue FROM orders WHERE status = 1 GROUP BY created_by ORDER BY total_revenue DESC`.
